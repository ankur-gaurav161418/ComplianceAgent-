...
def render_sidebar(chunks, ollama_online, model_ready):
    with st.sidebar:
        ...
        st.markdown(
            '<div class="ca-side-footer">Answers are sourced strictly from your indexed statutory '
            'documents and attachments, and can be exported as a signed opinion verified by '
            '{}.</div>'.format(esc(get_advocate()["name"])),
            unsafe_allow_html=True,
        )
...
def main():
    ...
    with page:
        ...
        st.markdown(
            '<div class="ca-disclaimer">Answers are drawn strictly from official statutory source '
            'documents and your attached files - not open-ended AI generation - and can be exported as '
            'an Advocate-verified legal opinion.</div>',
            unsafe_allow_html=True,
        )
